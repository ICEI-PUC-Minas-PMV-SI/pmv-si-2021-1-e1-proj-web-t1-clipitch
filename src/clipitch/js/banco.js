      ...
      if (cursor) {
        cursor.forEach((clip) => {
          ...
          element.innerHTML +=
            ...
            botao.setAttribute("onclick", `carregaModal("${clip["embed_url"]}","${clip.broadcaster["logo"]}","${clip.broadcaster["display_name"]}","${clip["title"]}","${clip["game"]}","${clip.curator["logo"]}","${clip.curator["display_name"]}","${CONST_PARENT}")`);
    ...
    if (rowDaily != null) {
      if (i <= 8) {
        rowDaily.innerHTML +=
          ...
          var botao = document.getElementById(url["slug"])
          botao.setAttribute("onclick", `carregaModal("${url["embed_url"]}","${url.broadcaster["logo"]}","${url.broadcaster["display_name"]}","${url["title"]}","${url["game"]}","${url.curator["logo"]}","${url.curator["display_name"]}","${CONST_PARENT}")`);
      }
